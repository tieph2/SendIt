...
import { getJudgesFromServer } from "@/Services/HttpClient.tsx";
...
export const Judge = () => {
	...
	const onPassButtonClick = () => {
		UpdateAttempt.send(currentRegistration.id, currentRegistration.boulder_id, true).catch(
			(err) => {
				console.error(err);
			}
		);
		PassService.send(currentRegistration.id, currentRegistration.boulder_id).catch((err) => {
			console.log("Error passing climber");
			console.error(err);
		});
		Dequeue.send(currentRegistration.id, currentRegistration.boulder_id).catch((err) => {
			console.error(err);
		});
		fetchRegistration();
	};
	...
	const onFailButtonClick = () => {
		UpdateAttempt.send(currentRegistration.id, currentRegistration.boulder_id, false).catch(
			(err) => {
				console.error(err);
			}
		);
	...
	return (
		<div className={"container"}>
			...
			{currentRegistration ? nextClimber : <p>Zone {zone} has no registered climber :(</p>}
